...
{	
	...
	private boolean optimumCheck(NetworkCell cell) {
		...
		{
			if((cell.getDimension(i) > (optimum[i]+optimumBound)) 
					|| (cell.getDimension(i) < (optimum[i]-optimumBound)))
			{
				return false;
			}
		}
	...
	private void checkNetworks() {
		if(rawFinalGenerationData.size() != ranNetworks.size() && rawGenerationData.size() != ranNetworks.size()
				&& !isSuppressErrors)
		{
			System.err.println("ERROR: Analyzer found inconsistencies in the received data and number of networks tested (number of runs).");
			System.exit(-1);
		}
